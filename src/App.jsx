  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      ...
      <style>
        ...
        <div class="footer">
          Report generated by YouTube Analytics AI Assistant &nbsp;|&nbsp; Powered by Claude API<br/>
  ...
  <div style={{ maxWidth: 640, margin: "0 auto" }}>
    ...
    <div style={{
      ...
      <p style={{ color: theme.textMuted, fontSize: 13, lineHeight: 1.7 }}>
        This app uses the <strong style={{ color: theme.text }}>Anthropic Claude API</strong> to power the conversational analytics.
        Claude analyzes your YouTube data and generates insights, answers questions, and creates reports.
      </p>
      <div style={{
        display: "flex", alignItems: "center", gap: 10, padding: 14,
        ...
        <span style={{ color: theme.blue, fontSize: 13, fontWeight: 500 }}>Claude API: Active</span>
      </div>
    </div>
  </div>
);
    ...
    try {
      const response = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [...messages.filter(m => m.role !== "assistant" || messages.indexOf(m) !== 0), { role: "user", content: userMsg }].slice(-10),
        }),
      });
      const result = await response.json();
      const text = result.content?.map(b => b.text || "").join("") || "Sorry, I couldn't process that. Please try again.";
          ...
          <div style={{ animation: "fadeSlideUp 0.3s ease" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28 }}>
              <div>
                ...
                <p style={{ color: theme.textMuted, fontSize: 13, marginTop: 4 }}>Last 30 days overview</p>
              </div>
                ...
                {TOP_VIDEOS.map((v, i) => (
                  <div key={i} style={{
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <div>
                        <div style={{ color: theme.text, fontSize: 13, fontWeight: 500 }}>{v.title}</div>
